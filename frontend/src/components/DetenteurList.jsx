    ...
    <div className="detenteurs-grid">
      {dets.length === 0 ? (
        <div style={{gridColumn: '1/-1', textAlign: 'center', padding: '60px 20px', background: 'white', borderRadius: '12px', boxShadow: '0 2px 8px rgba(139,69,19,0.1)'}}>
          ...
          <h3 style={{color: 'var(--terre)', marginBottom: '10px'}}>Aucun dÃ©tenteur enregistrÃ©</h3>
          <p style={{color: 'var(--gris)'}}>Commencez par crÃ©er votre premier dÃ©tenteur dans le menu "Nouveau dÃ©tenteur"</p>
        </div>
      ) : (
        dets.map(d => (
          <div key={d.id || d.reference} className="detenteur-card">
            <div className="detenteur-card-header">
              <h3>{d.typePersonne === 'InvitÃ©' ? 'ðŸ¤' : d.typePersonne === 'Visiteur' ? 'ðŸš¶' : d.typePersonne === 'Spectateur' ? 'ðŸ‘ï¸' : 'ðŸŽµ'} {d.nomComplet}</h3>
              <div className="village">ðŸ“ Originaire de {d.village}</div>
              {d.lieuSignature && <div style={{fontSize: '11px', opacity: 0.9, marginTop: '3px'}}>âœï¸ SignÃ© Ã  {d.lieuSignature}</div>}
              {d.syncStatus === 'synced' && (
                <span className="sync-badge">âœ… SYNC</span>
              )}
            </div>
            <div className="detenteur-card-body">
              ...
              <div className="info-row">
                ...
                <span className="info-value">{d.sexe === 'M' ? 'â™‚ Masculin' : 'â™€ FÃ©minin'}</span>
              </div>
              {d.fonctionPalais && (
                <div className="info-row">
